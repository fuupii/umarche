            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form id="delete_{{ $image->id }}" method="POST" action="{{ route('owner.images.destroy', ['image' => $image->id])}}">
                        ...
                        @method('delete')
                            <div class="flex mt-4 p-2 w-full justify-around">
                            <a href="#" data-id="{{ $image->id }}"onclick="deletePost(this)" type="submit" class="text-white bg-red-400 border-0 py-2 px-8 focus:outline-none hover:bg-red-500 rounded ">削除する</a>
                            </div>
                    </form>
                </div>
            </div>
